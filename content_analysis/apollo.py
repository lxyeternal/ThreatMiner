...
def user_name():
    ...
    username_list = []
    forums_name_list = ['0x00sec', 'garage4hackers', 'hacktoday', 'SafeSkyHacks.com', 'hackthissite', 'antionline', 'sky-fraud.ru']
...
def forums_name():
    ...
    forums_user = "select distinct forum from forums"
    forum_list = []
    result = sql_inquire(forums_user)
    for i in result:
...
def length_topic():
    ...
    for forum in forums_name_list:
        ...
        for username in username_list[forum_index]:
            ...
            username = username.replace("'", "\\'")
            username = username.replace('"', "\\'")
            sql = "select content from forums where user = '%s' and isorigin = 1 and forum = '%s'" % (username,forum)
            result = sql_inquire(sql)
            if(len(result) == 0):
                lg_topic = 0
            else:
            ...
            update_sql = "update indicators set length_topics = '%s' where user_name = '%s' and forums = '%s'" %(lg_topic,username,forum)
            store_forum(update_sql)
    ...
    for forum in forums_name_list:
        ...
        for username in username_list[forum_index]:
            ...
            if(len(result) == 0):
                ...
                lg_topic = format(float(all_lg) / float(num), '.2f')
            print(lg_topic)
            update_sql = "update indicators set length_replies = '%s' where user_name = '%s' and forums = '%s'" %(lg_topic,username,forum)
            store_forum(update_sql)
...
def length_difference():
    ...
    for forum in forums_name_list:
        ...
        for i in result:
            ...
            username = username.replace("'", "\\'")
            username = username.replace('"', "\\'")
            if(i[1] == '0'  or i[1] == '0.00'):
                length_df = 1
            elif((i[1] == '0'  or i[1] == '0.00') and (i[2] == '0'  or i[2] == '0.00')):
                length_df = 0
            else:
